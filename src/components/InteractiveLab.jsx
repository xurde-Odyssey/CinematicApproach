...
const LabCard = ({ title, icon, color, onClick, description, index }) => {
    ...
    return (
        <div
            ref={cardRef}
            onClick={onClick}
            className="prismatic-card group perspective-1000"
            style={{
                height: '320px',
                cursor: 'pointer',
                '--accent-inner': color,
        ...
        >
            <div className="card-inner relative w-full h-full transition-all duration-500 preserve-3d">
                <div className="absolute inset-0 bg-[var(--text-primary)]/[0.03] backdrop-blur-xl border border-[var(--text-primary)]/10 rounded-3xl overflow-hidden shadow-2xl transition-all duration-500 group-hover:bg-[var(--text-primary)]/[0.07] group-hover:border-[var(--text-primary)]/20">
                    ...
                    <div className="p-10 h-full flex flex-col items-center justify-center text-center space-y-6">
                        ...
                        <p className="text-xs text-[var(--text-primary)]/40 leading-relaxed font-sans max-w-[180px] opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-500">
                            {description}
                        </p>
                    </div>
...
const Book = ({ title, author, content, onClose, type, onNext }) => {
    ...
    return (
        <div className="fixed inset-0 z-[3000] flex items-center justify-center p-6">
            ...
            <div ref={bookRef} className="relative w-full max-w-4xl bg-[#0a0a0a] border border-white/10 rounded-3xl overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.8)] h-[80vh] flex flex-col md:flex-row">
                {/* Close Button */}
                <button onClick={onClose} className="absolute top-6 right-6 z-10 text-white/30 hover:text-white transition-colors p-2">
                    <X size={24} />
                </button>

                {/* Left Side: Meta */}
                <div className="w-full md:w-1/3 bg-white/[0.02] border-r border-white/5 p-12 flex flex-col justify-center">
                    <div className="space-y-8">
                        ...
                        <p className="text-white/40 font-mono text-sm tracking-widest italic">by {author}</p>
                ...
                {/* Right Side: Content */}
                <div className="w-full md:w-2/3 p-12 md:p-20 overflow-y-auto custom-scrollbar">
                    <div className="max-w-prose mx-auto">
                        <p className="text-lg md:text-xl text-white/80 leading-relaxed font-serif whitespace-pre-wrap italic">
                            {content}
                        </p>
                    </div>
                </div>
...
const FavoritesView = () => {
    ...
    return (
        <div className="w-full max-w-5xl space-y-20 py-10">
...
const InteractiveLab = () => {
    ...
    return (
        <section ref={sectionRef} className="relative min-h-screen py-40 px-6 flex flex-col items-center bg-[var(--bg-color)] overflow-hidden transition-colors duration-700">
            {/* High-End Ambient Lighting */}
            <div className="ambient-light-1 absolute top-1/4 left-1/4 w-[600px] h-[600px] bg-blue-500/5 blur-[150px] rounded-full pointer-events-none" />
            <div className="ambient-light-2 absolute bottom-1/4 right-1/4 w-[800px] h-[800px] bg-purple-500/5 blur-[180px] rounded-full pointer-events-none" />
            <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-[0.03] pointer-events-none" />

            <div className="lab-heading text-center mb-32 z-10 space-y-6">
                <div className="flex items-center justify-center gap-4 text-[var(--text-primary)]/30">
                    <div className="w-12 h-[1px] bg-current" />
                    <span className="font-mono text-[10px] uppercase tracking-[0.5em]">// Experimental_Subsystem</span>
                    <div className="w-12 h-[1px] bg-current" />
                </div>

                <h2 className="text-6xl md:text-8xl lg:text-9xl font-black tracking-tighter text-[var(--text-primary)] uppercase leading-none">
                ...
                <p className="text-[var(--text-secondary)] max-w-xl mx-auto font-serif italic text-lg leading-relaxed pt-4">
                    Where abstract concepts meet digital interaction. A curated space for experiments and creative detours.
                </p>
            </div>

            <div className="lab-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 w-full max-w-7xl z-10 px-4">
                {tools.map((tool, idx) => (
                ...
                <div className="fixed inset-0 z-[2000] flex items-center justify-center p-6">
                    ...
                    {['wheel', 'visual'].includes(activeTool.id) && (
                        <div className="relative w-full max-w-5xl h-[80vh] flex items-center justify-center bg-[#070707] border border-white/10 rounded-3xl overflow-hidden shadow-2xl">
                            <button onClick={() => setActiveTool(null)} className="absolute top-8 right-8 z-[3000] text-white/30 hover:text-white transition-colors p-2">
                                <X size={32} />
                            </button>
                                ...
                                <div className="text-center space-y-12">
                                    <div className="relative w-32 h-32 mx-auto">
                                        ...
                                        <div className="relative z-10 h-full w-full rounded-3xl bg-white/5 border border-white/10 flex items-center justify-center">
                                            {React.cloneElement(activeTool.icon, { size: 48, className: "text-blue-500" })}
                                        </div>
                                    </div>
                                    <div className="space-y-4">
                                        <h2 className="text-5xl font-black text-white uppercase tracking-tighter">Under Calibration</h2>
                                        <p className="font-mono text-[10px] text-blue-500/50 uppercase tracking-[0.5em]">[ Code_Status: Refactoring_In_Progress ]</p>
                                    </div>
                                </div>
                            )}
                        </div>
